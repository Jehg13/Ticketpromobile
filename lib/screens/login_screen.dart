...
class _LoginScreenState extends State<LoginScreen> {
  ...
  int segundosBloqueo = 0;
  Timer? _temporizadorBloqueo;
  ...
  Future<void> iniciarSesion() async {
    if (iniciandoSesion || segundosBloqueo > 0) return;

    final String usuario = usuarioController.text.trim();
    final String password = passwordController.text;

    if (usuario.isEmpty) {
      mostrarMensaje('Ingresa tu usuario o correo electrónico.');
      return;
    }
    ...
    try {
      ...
      if (mfaRequired) {
        ...
        final challenge =
            (resultado['mfa_challenge'] ??
                    (respuestaApi is Map
                        ? respuestaApi['mfa_challenge']
                        : null))
                ?.toString()
                .trim() ??
            '';
      ...
      final String? token = resultado['token']?.toString();

      if (token == null || token.isEmpty) {
        mostrarMensaje('El servidor no devolvió un token de acceso.');
        return;
      }

      final dynamic usuarioDatos = resultado['user'];

      if (usuarioDatos is! Map<String, dynamic>) {
        mostrarMensaje('No se recibieron los datos del usuario.');
        return;
      }

      await SessionService.saveSession(token: token, user: usuarioDatos);
      await NotificationService.initialize();

      if (!mounted) return;

      final String privAdmin =
          usuarioDatos['priv_admin']?.toString().trim().toUpperCase() ?? 'N';
  ...
  String _mensajeLogin(Map<String, dynamic> resultado) {
    final retryAfter = _segundosDesde(resultado['retry_after']);
    if (resultado['statusCode'] == 429 || retryAfter > 0) {
      return 'Demasiados intentos. Podrás volver a intentarlo en '
          '${_formatearTiempo(retryAfter)}.';
    }

    return resultado['message']?.toString() ?? 'No fue posible iniciar sesión.';
  }

  int _segundosDesde(dynamic value) {
  ...
  String _formatearTiempo(int segundos) {
    if (segundos >= 60) {
      ...
      return restantes == 0 ? '$minutos min' : '$minutos min ${restantes}s';
    }
  ...
  void _activarBloqueo(dynamic valor) {
    ...
    _temporizadorBloqueo = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      if (segundosBloqueo <= 1) {
        _temporizadorBloqueo?.cancel();
        setState(() {
          segundosBloqueo = 0;
        });
              ...
              child: IgnorePointer(
                child: Container(
                  ...
                  decoration: const BoxDecoration(
                    shape: BoxShape.circle,
                    gradient: RadialGradient(
                      colors: [Color(0x222F7DFF), Colors.transparent],
                    ),
                  ),
                ),
              ),
            ...
            SafeArea(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final double contentWidth = tablet ? 560 : width;
                        ...
                        child: SizedBox(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                _buildAccessPill(),
                                SizedBox(height: veryShortScreen ? 10 : 14),
                                _buildLogo(smallPhone: smallPhone),
                                ...
                                _buildLoginButton(smallPhone: smallPhone),
                                SizedBox(height: veryShortScreen ? 12 : 17),
                                _buildSecurityCard(smallPhone: smallPhone),
                                SizedBox(height: veryShortScreen ? 10 : 17),
                                const Text(
  ...
  Widget _buildLogo({required bool smallPhone}) {
    final double logoTextSize = smallPhone ? 31 : 37;
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: [Color(0xFF4A94FF), Color(0xFF174FAE)],
            ...
            boxShadow: [
              BoxShadow(
                color: const Color(0xFF1677FF).withValues(alpha: 0.34),
                blurRadius: 24,
                spreadRadius: 1,
  ...
  Widget _buildAccessPill() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF102653).withValues(alpha: 0.58),
        borderRadius: BorderRadius.circular(30),
        border: Border.all(
          color: const Color(0xFF75A1FF).withValues(alpha: 0.18),
        ),
        boxShadow: [
          BoxShadow(
      ...
      child: const Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(LucideIcons.shield_check, color: Color(0xFF70A5FF), size: 14),
          SizedBox(width: 7),
          Text(
            'ACCESO SEGURO',
            style: TextStyle(
  ...
  Widget _buildFormCard({required bool smallPhone, required bool shortScreen}) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.all(smallPhone ? 14 : 18),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFF142E62).withValues(alpha: 0.72),
            const Color(0xFF09152F).withValues(alpha: 0.86),
          ],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        ...
        border: Border.all(
          color: const Color(0xFF75A1FF).withValues(alpha: 0.16),
        ),
        boxShadow: [
          BoxShadow(
          ...
          BoxShadow(
            color: const Color(0xFF1677FF).withValues(alpha: 0.07),
            blurRadius: 35,
            spreadRadius: -10,
          ),
        ],
      ),
      child: _buildFormulario(smallPhone: smallPhone, shortScreen: shortScreen),
    );
  }
  ...
  Widget _buildTextField({
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: Icon(icon, color: const Color(0xFF4A94FF), size: 19),
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
          borderSide: const BorderSide(color: Color(0xFF4A94FF), width: 1.4),
        ),
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: IconButton(
          ...
          icon: Icon(
            mostrarPassword ? LucideIcons.eye_off : LucideIcons.eye,
  ...
  Widget _buildLoginButton({required bool smallPhone}) {
    return Container(
      width: double.infinity,
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF1677FF).withValues(alpha: 0.28),
            blurRadius: 24,
            spreadRadius: -3,
            offset: const Offset(0, 10),
          ),
      ...
      child: ElevatedButton(
        onPressed: iniciandoSesion || segundosBloqueo > 0
            ? null
            : iniciarSesion,
        ...
        child: iniciandoSesion
            ...
            ? Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Icon(LucideIcons.clock_3, size: 18),
                  const SizedBox(width: 9),
                  Text(
                    'ESPERA ${_formatearTiempo(segundosBloqueo)}',
                    style: const TextStyle(
                      fontSize: 13.5,
                      fontWeight: FontWeight.w700,
                      letterSpacing: 0.4,
  ...
  Widget _buildSecurityCard({required bool smallPhone}) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.all(smallPhone ? 12 : 14),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFF0C1D3F).withValues(alpha: 0.90),
            const Color(0xFF102653).withValues(alpha: 0.58),
          ],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(17),
        border: Border.all(
          color: const Color(0xFF4A94FF).withValues(alpha: 0.13),
        ),
      ),
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: const Color(0xFF4A94FF).withValues(alpha: 0.08),
              ),
            ),
            child: Icon(
              LucideIcons.shield_check,
          ...
          Container(
            ...
            decoration: const BoxDecoration(
              ...
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: Color(0x8857D697), blurRadius: 9)],
            ),
          ),
        ],
      ),
